...
int main(int argc, char **argv)
{
    ...
    struct filter_rule filter1[3] = {
        { data->offsets.srcport, 443, 0, filter_eq_uint16_t },
        { data->offsets.dstport, 25, 0, filter_eq_uint16_t },
        { 0, 0, 0, NULL}
    };

    struct filter_rule filter2[3] = {
        { data->offsets.srcport, 443, 0, filter_eq_uint16_t },
        { data->offsets.srcport, 80, 0, filter_eq_uint16_t },
        { 0, 0, 0, NULL}
    };

    struct filter_rule *filter_rules[2] = {
        filter1,
        filter2
    };

    filtered_records = filter(data, filter_rules, 2, &num_filtered_records);
